...
export function getMealsForUser(userProfile: UserProfile): Meal[] {
  // Filter and customize meals based on user profile
  let filteredMeals = [...allMeals];

  // Filter by dietary preference
  if (userProfile.dietPreference === 'Vegan') {
    filteredMeals = filteredMeals.filter(meal => 
      meal.tags.includes('vegan') || meal.tags.includes('vegetarian')
    );
  } else if (userProfile.dietPreference === 'Vegetarian') {
    filteredMeals = filteredMeals.filter(meal => 
      !meal.tags.includes('meat') && !meal.tags.includes('seafood')
    );
  } else if (userProfile.dietPreference === 'Pescatarian') {
    filteredMeals = filteredMeals.filter(meal => 
      !meal.tags.includes('meat')
    );
  }

  // Customize recommendations based on medical conditions
  filteredMeals = filteredMeals.map(meal => {
    let customMeal = { ...meal };
    
    if (userProfile.medicalConditions.includes('Hypertension')) {
      if (meal.tags.includes('low-sodium')) {
        customMeal.whyRecommended = meal.whyRecommended || 'Low in sodium, suitable for managing hypertension';
      }
    }
    
    if (userProfile.medicalConditions.includes('Diabetes Type 2')) {
      if (meal.tags.includes('low-carb') || meal.carbs < 40) {
        customMeal.whyRecommended = meal.whyRecommended || 'Lower in carbs to help manage blood sugar levels';
      }
    }
